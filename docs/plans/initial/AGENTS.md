...
6. **Task outputs match the roadmap's release scope.** When a task file's `Outputs` or `Acceptance criteria` reference release-artifact specifics (binaries, tarballs, signed releases, distribution channels, multi-platform matrices), cross-reference `VEKTOR_ROADMAP.md`'s Stage table for the release the task targets. If they disagree, fix one — never let both stand. Example of the kind of drift this catches: a Phase 1 task claiming "ship signed cross-platform binaries with checksums" when the roadmap explicitly defers binary releases to Phase 6 / task 6.2. The roadmap is the canonical source for what each release contains.
7. **A commit cannot reliably contain its own hash.** Schemes like `git commit ... && sed s/PLACEHOLDER/$(git rev-parse HEAD)/ ... && git commit --amend` produce a stale hash because the amend changes the hash. Don't try. Use a stable marker (`(release commit)`, `(this commit)`) and rely on `git log` to identify the commit post-hoc.
