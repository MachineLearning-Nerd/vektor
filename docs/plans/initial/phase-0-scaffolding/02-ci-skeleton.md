...
- **protoc requirement (2026-05-28 incident)**: First CI run on `73ed9d9` failed at `cargo clippy` because `lance-encoding v6.0.0`'s `build.rs` invokes `protoc` to generate Rust from `.proto` files. CI runners (ubuntu-latest, macos-latest) don't ship with protoc; local dev machines often have it brew-installed silently. **Fix**: add `arduino/setup-protoc@v3` step right after Rust toolchain install. Any future workflow that runs `cargo build/check/test/clippy` against this stack needs the same step. The audit.yml workflow does NOT need protoc — cargo-deny inspects metadata without compiling. **Lesson**: third "local env vs CI env" gap this session (after MSRV mismatch and cfg_select). When writing a CI workflow that compiles a Rust project, list every system-package dependency before merging.
- **Node.js 20 deprecation warning**: `actions/checkout@v4` and `arduino/setup-protoc@v3` still use Node.js 20 runners. GitHub will force Node.js 24 on 2026-06-02 and remove Node.js 20 on 2026-09-16. Pin newer action versions when they ship Node 24 support, or set `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` to opt in early. Not a blocker today; tracking for the v0.4 hardening pass.
- **First-run wall-clock**: 17 minutes per platform (macOS-latest + ubuntu-latest) on cold cache. Swatinem cache cuts this to ~3-5 min on subsequent runs. Worth knowing for release-time expectations.
