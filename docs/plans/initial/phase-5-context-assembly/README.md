...
**Status**: ✅ Done — implementation complete and verified. Real-model first-query latency remains manual/model-gated per Task 5.12/5.13 notes.
...
- [x] All 13 tasks above marked ✅ Done (5.10/5.11 are folded into 4.4 — they're listed for cross-reference only)
- [x] `get_context_for_prompt` returns the full `ContextPackage` JSON per PRD §5.3 with: `chunks`, `files_included`, `total_tokens`, `budget_used_pct`, `missing_context_warnings`, `result_confidence`, `budget_gap_reason`, `clusters`
- [x] Token budget: requesting `token_budget=8000` stays within the requested budget via two-pass verification; the near-target fill behavior is covered at the assembler level.
- [x] Dedup: a sliding-window fixture with 80% overlap produces merged chunks.
- [x] Related expansion: a query that hits `src/auth/jwt.py` includes chunks from `tests/test_auth.py` if such a test file exists.
- [x] Recency: a recently-edited file ranks higher than an older file with the same RRF score; but a recently-edited *irrelevant* file does NOT pollute top-5 (min-score gate).
- [x] Shallow indexing: the shallow tier writes searchable BM25 docs before deep indexing, and the MCP integration verifies indexed context through the full server path.
- [x] IndexStatusTracker: while shallow indexing runs, handlers return `index_status: "partial"` with results; after deep completes, they return `"full"`.
- [x] Warm-up: `vektor serve` runs batch-1 and batch-32 warm-up before serving; real-model `<150ms` first-query timing remains manual/model-gated.
- [x] CI green; no regression on Phase 4 search latency.
